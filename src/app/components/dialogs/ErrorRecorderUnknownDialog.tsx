...
const ErrorRecorderUnknownDialog = () => {
    ...
    useEffect(() => {
        ...
        if(audioRecorderHasUnknownError && !dialog.open) {
            dialog.showModal();
        } else if(!audioRecorderHasUnknownError && dialog.open) {
            dialog.close();
        }
    ...
    return (
        <dialog ref={dialogRef} className="modal">
            <div className="modal-box">
                ...
                <form method="dialog">
                    <button className="btn btn-sm btn-circle btn-ghost absolute right-2 top-2" onClick={() => closeAudioRecorderUnknownError()}>✕</button>
                </form>
                <div className="flex flex-col">
                    <div className="mt-3">
                        <p className="py-4 flex items-center gap-x-4">
                            ...
                            <span className="shrink">{t("dialogs.errorRecorderUnknownDialog.info")}</span>
                        </p>
                    </div>
                </div>
                <div className="modal-action mt-0">
                    <form method="dialog">
                        <button className="btn" onClick={() => closeAudioRecorderUnknownError()}>{t("ok")}</button>
                    </form>
                </div>
            </div>
        </dialog>
...
export default ErrorRecorderUnknownDialog;
